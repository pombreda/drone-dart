...
func (gsa *Client) PutObject(obj *Object, content io.ReadCloser) (shouldRetry bool, err error) {
	...
	req.Header.Set("Cache-Control", "public, max-age=0, no-transform")

	// determine content type settings
	switch {
	case strings.HasSuffix(obj.Key, ".json"):
		req.Header.Set("Content-Type", "application/json")
	case strings.HasSuffix(obj.Key, ".txt"):
